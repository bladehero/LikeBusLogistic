    ...
    $.ajaxSetup({
        ...
        complete: function (xhr) {
            ...
            if (xhr.status === 401) {
                Swal.fire({
                    title: 'Exit',
                    html: 'Your session has expired! <br /> Please re-login.',
                    icon: 'info'
                }).then(() => {
                    $('#logout').click();
                });
...
var App = {
    ...
    footer: {
        ...
        getContent: function (url, data, finished) {
            ...
            $.get(url, data, function (html) {
                ...
                App.footer.content.html('An unexpected error has occurred!');
            }).then(function () {
                if (finished) {
                    finished();
                }
    ...
    useContentState: function () {
        var _showFunc = () => {
            App.footer.content.html('<div class="uk-text-center">Welcome to the Logistic app! <br /> Select one of the menu items to get started. </div>');
            App.menu.show(1000);
            App.footer.show();
        };
        var state = App.getLastContentState();
            ...
            success: function (result) {
                if (result.success) {
                    if (successHandler) {
                        ...
                        App.message.showSuccessWithOk('Success', result.message || 'Successfully completed!');
                    }
                } else {
                    if (errorHandler) {
                        errorHandler(result);
                    } else {
                        App.message.showErrorWithOk('Error', result.message || 'An unexpected error has occurred!');
                    }
                }
            }
        });
    ...
    loadContent: function (selector, url, data, handler, finished) {
        ...
        $.get(url, data, function (html) {
            ...
            _content.html('An unexpected error has occurred!');
        }).then(function () {
            if (finished) {
                finished();
            }
...
$(document).ready(function () {
    ...
    $(document).on('fullscreenchange', function () {
        ...
        if (fullscreenMode) {
            _this.html('<span class="uk-margin-small-right" uk-icon="icon: shrink"></span> Normal screen');
        } else {
            _this.html('<span class="uk-margin-small-right" uk-icon="icon: expand"></span> Full screen');
        }
    });
